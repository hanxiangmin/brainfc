...
从原始扫描开始，请先读 [原始 fMRI → 脑网络：完整流程与参数](python-preprocessing.md)，核对空间预处理、去噪、相关估计和质控，再按本页构建网络。
...
4. 在结果中切换节点连线、透明包络、脑区分区表面、矩阵和完整成员表。影像提取会携带所用图谱的分区表面；旧本地结果在原图谱仍存在、校验值和脑区顺序一致时自动恢复。只有缺少可核实分区数据时按钮才不可选，并显示原因；原始图谱切片仍需要已安装的图谱数据。

选中脑区、普通连接或超边后，自动标出对应脑区的简短名称；鼠标悬停查看完整名称。“显示前侧脑区名称”可补充当前视角朝向相机一侧的名称，避免后侧文字遮挡。名称缩写不改变结果中的 ROI 名称或编号。
