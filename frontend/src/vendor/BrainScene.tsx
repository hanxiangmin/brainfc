...
export type SceneHandle = {
  image: () => Promise<Blob>;
  views: (cameras: string[][], title: string) => Promise<Blob>;
  camera: () => CameraState;
...
function meshGeometry(s: Surface, smooth = false) {
  // Smooth only the display envelope. Source geometry and ROI coordinates are immutable.
  let positions = new Float32Array(s.positions);
  if (smooth && positions.length) {
    const neighbors = Array.from(
      { length: positions.length / 3 },
      () => new Set<number>(),
    );
    for (let i = 0; i < s.indices.length; i += 3) {
      const triangle = s.indices.slice(i, i + 3);
      triangle.forEach((a) =>
        triangle.forEach((b) => {
    ...
    for (let iteration = 0; iteration < 12; iteration++) {
      const next = positions.slice(),
        amount = iteration % 2 ? -0.53 : 0.5;
      neighbors.forEach((adj, i) => {
        if (!adj.size) return;
        for (let k = 0; k < 3; k++) {
          let mean = 0;
          adj.forEach((j) => (mean += positions[j * 3 + k]));
          next[i * 3 + k] += amount * (mean / adj.size - positions[i * 3 + k]);
        }
      });
      positions = next;
    }
  ...
  if (smooth) {
    ...
    // Reflections in voxel-to-world affines can reverse the surface winding.
    if (volume < 0)
      for (let i = 0; i < indices.length; i += 3)
  ...
  function BrainScene(props, ref) {
    ...
    useImperativeHandle(ref, () => ({
      ...
      views: async (cameras, title) => {
        ...
        c.height = 1390;
        const ctx = c.getContext("2d")!,
          dark = current.current.view.theme === "midnight";
        ctx.fillStyle = dark ? "#111d2e" : "#f8fafc";
        ctx.fillRect(0, 0, c.width, c.height);
        ctx.fillStyle = dark ? "#deebf7" : "#23364b";
        ctx.textAlign = "center";
        ctx.font = "28px Segoe UI,sans-serif";
        ctx.fillText(title, 1200, 44);
        try {
          cameras.forEach(([key, label], i) => {
            r.orient(key);
            r.render();
            const x = (i % 4) * 600,
              y = 80 + Math.floor(i / 4) * 650;
            ctx.fillStyle = dark ? "#deebf7" : "#23364b";
            ctx.font = "24px Segoe UI,sans-serif";
            ctx.fillText(label, x + 300, y + 28);
            ctx.drawImage(r.renderer.domElement, x, y + 50, 600, 560);
            ctx.drawImage(overlay.current!, x, y + 50, 600, 560);
          });
        } finally {
          r.restore(saved);
          r.render();
        ...
        return new Promise((resolve, reject) =>
          c.toBlob(
            (b) => (b ? resolve(b) : reject(new Error("八视图导出失败"))),
            "image/png",
          ),
        );
      },
    ...
    useEffect(() => {
      ...
      const scene = new T.Scene(),
        ...
        group = new T.Group();
      const coordinates = current.current.geometry.brain.positions.length
        ? current.current.geometry.brain.positions
        : current.current.rois.flatMap((roi) => roi.coordinates);
      const bounds = new T.Box3().setFromArray(coordinates),
      ...
      const r: Runtime = {
        ...
        orient: (view) => {
          ...
          const v: Record<string, number[]> = {
            ...
            superior: [0, 0, 1],
            inferior: [0, 0, -1],
            "left-oblique": [-0.62, 0.62, 0.47],
            "right-oblique": [0.62, 0.62, 0.47],
            reset: [0.56, 0.72, 0.46],
          };
          camera.up.set(
            0,
            view === "superior" || view === "inferior" ? 1 : 0,
            view === "superior" || view === "inferior" ? 0 : 1,
          );
      ...
      resize();
      const isFront = (xyz: number[]) =>
        new T.Vector3()
          .fromArray(xyz)
          .sub(center)
          .dot(camera.position.clone().sub(center).normalize()) >= 0;
      const drawLabels = () => {
        ...
        if (p.view.labels)
          for (const roi of ordered.filter(
            (roi) =>
              (roi.source_roi_id || roi.roi_id) === r.hovered &&
              isFront(roi.coordinates),
          )) {
            ...
            const tw = ctx.measureText(shortROI(roi.abbreviation)).width + 7;
            ...
            ctx.fillStyle =
              p.view.selection?.id === id && p.view.selection.kind === "node"
                ...
                : dark
                  ? "#deebf7"
                  : "#23364b";
            ctx.fillText(shortROI(roi.abbreviation), x, y);
            if (r.hovered === id) {
        ...
        for (const [label, xyz] of [
          ...
          ctx.fillText(
            label,
            Math.max(12, Math.min(w - 20, v.x)),
            Math.max(22, Math.min(h - 42, v.y)),
          );
        }
        ...
        ctx.fillText(
          `${p.geometry.space} · RAS+ · mm   |   ${p.rois.length} ROI   |   Hyper-Brain`,
          18,
          h - 18,
        );
        element.dataset.labelCount = String(r.labelHits.length);
      ...
      const hits = (e: PointerEvent) => {
        ...
        for (const hit of ray.intersectObjects(r.pickables, false)) {
          ...
          if (s?.kind === "node") {
            const roi = current.current.rois.find(
              (n) => (n.source_roi_id || n.roi_id) === s.id,
            );
            if (!roi || !isFront(roi.coordinates)) continue;
          }
          if (
            s &&
            v.only_selected &&
        ...
        return found.sort(
          (a, b) => Number(a.kind !== "node") - Number(b.kind !== "node"),
        );
      };
      let down = { x: 0, y: 0 },
        throttle = 0;
      ...
      const move = (e: PointerEvent) => {
        ...
        if (!s || (s.kind === "node" && !p.view.labels)) {
          setHover(null);
          return;
        }
        const text =
        ...
        setHover({
          text,
          x: Math.max(8, Math.min(e.clientX - b.left + 12, b.width - 300)),
    ...
    useEffect(() => {
      ...
      const brain = new T.Mesh(
        meshGeometry(geometry.brain, true),
        new T.ShaderMaterial({
          uniforms: {
            alpha: { value: view.opacity },
            tint: { value: new T.Color("#93b0bc") },
          },
          vertexShader: `varying vec3 n; varying vec3 v; void main(){
            vec4 p=modelViewMatrix*vec4(position,1.0); n=normalize(normalMatrix*normal); v=normalize(-p.xyz);
            gl_Position=projectionMatrix*p; }`,
          fragmentShader: `uniform float alpha; uniform vec3 tint; varying vec3 n; varying vec3 v;
            void main(){ float rim=pow(1.0-abs(dot(normalize(n),normalize(v))),2.0);
              float light=0.72+0.28*max(dot(normalize(n),normalize(vec3(-0.4,0.7,1.0))),0.0);
              gl_FragColor=vec4(tint*light,alpha*(0.1+0.9*rim));
      ...
      const sphere = new T.SphereGeometry(1, 24, 16),
        nodeMesh = new T.InstancedMesh(
          sphere,
          transparentMaterial(
            new T.MeshStandardMaterial({ roughness: 0.32, metalness: 0.08 }),
          ),
          rois.length,
        );
          ...
          edges.forEach((e) => {
            ...
            if (a && b)
              segment(
                ...
                b,
                { kind: "edge", id: e.id },
                new T.Color(e.weight < 0 ? "#438fb7" : "#cf8858"),
              );
          });
        else {
    ...
    useEffect(() => {
      ...
      if (r.brain) {
        const material = r.brain.material as T.ShaderMaterial;
        material.uniforms.alpha.value = view.opacity;
        material.uniforms.tint.value.set(dark ? "#8ea8ba" : "#7595a7");
